#include "Heroes_PopulateSpawnList.as"
		...
		{	 
			...
			if (menu !is null)
			{
				...
				for (uint i=0; i < respawns.length; i++)
				{
					...
					string respawnString = "Spawn at " + respawnLoc;
					CGridButton@ button2 = menu.AddButton( "$"+respawnName+"$", respawnString + respawn.getInventoryName(), this.getCommandID("pick spawn"), Vec2f(BUTTON_SIZE, BUTTON_SIZE), params );
					if (button2 !is null)
					{
...
void onCommand( CRules@ this, u8 cmd, CBitStream @params )
{
    if (cmd == this.getCommandID("pick spawn"))    {
		...
		if (cmd == this.getCommandID("pick default"))	
		{
