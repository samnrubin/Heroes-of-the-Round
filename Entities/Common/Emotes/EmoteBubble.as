...
{		
   ...
   // if (blob.exists("emote"))	 will show skull if none existant
	if (!blob.getShape().isStatic())
    {
		...
		{
			if(blob.getPlayer() !is null && (!blob.hasTag("emoted") || blob.get_u8("lastemote") != index )){
				blob.Tag("emoted");
				blob.set_u8("lastemote", index);
				handleEmote(blob, index);
			}
