	...
	{			
		...
		{						 
				...
				)
			{
				...
				{
					string display = mouseBlob.getInventoryName();
					if(mouseBlob.hasTag("Displayhealth")){
						string currentHealth = formatFloat(mouseBlob.getHealth(), "", 0, 2);
						string initialHealth = formatFloat(mouseBlob.getInitialHealth(), "", 0, 2);
						display += "\n" + currentHealth + "/" + initialHealth;
					}
					GUI::DrawText( display, mouseBlob.getScreenPos() + Vec2f(-mouseBlob.getWidth(), mouseBlob.getHeight() + 16.0f), color_white );
