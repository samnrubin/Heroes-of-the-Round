...
bool isBuildableAtPos( CBlob@ this, Vec2f p, TileType buildTile, CBlob @blob, bool &out sameTile )
{
    ...
    else // BLOB
    {
		...
		isWaypoint = blob.getName() == "waypoint";
    }
...
//printf("a");
    ...
    if (!isWaypoint && (buildTile > 0 && buildTile < 255 && blob is null && buildTile == map.getTile(offset).type)) {
		sameTile = true;
        return false;
    }		
...
//printf("b");
	if (backtile.type == CMap::tile_wood && buildTile == CMap::tile_castle)
	{
					...
					{	
						...
						else 
						{
							...
							if (cantBuild &&
								...
								bname != "bush" &&
									(
										(middle.x > bpos.x - b.getWidth()*0.5f) && (middle.x < bpos.x + b.getWidth()*0.5f) 
										&& (middle.y > bpos.y - b.getHeight()*0.5f) && (middle.y < bpos.y + b.getHeight()*0.5f)
									)
								)
							{
								return false;
