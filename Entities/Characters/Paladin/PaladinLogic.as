...
void onTick( CBlob@ this )
{
	...
	if(this.isKeyJustPressed(key_taunts) && (getGameTime() - knight.forceAbilityTimer) > KnightVars::force_ability_time){
		forceAway(this);
		knight.forceAbilityTimer = getGameTime();
	}
}

void forceAway(CBlob@ this){
	if(getNet().isServer()){
		Vec2f pos = this.getPosition() + Vec2f(0, t(0.5));
		CBlob@[] hitBlobs;
		f32 radius = t(7);
		this.getMap().getBlobsInRadius(pos, radius, @hitBlobs);
	...
	this.getSprite().PlaySound("MigrantSayHello.ogg", 5.0f);
...
void DoAttack(CBlob@ this, f32 damage, f32 aimangle, f32 arcdegrees, u8 type, int deltaInt, PaladinInfo@ info )
{
	if (!getNet().isServer()) {
		return;
	}
...
bool isSliding(PaladinInfo@ knight)
{
