...
{
	if (caller.getTeamNum() == this.getTeamNum() && this.hasTag("travel tunnel") 
...
bool getTunnelsForButtons( CBlob@ this, CBlob@[]@ tunnels )
{
	...
	{
		CBlob@ blob = list[i];
		if (blob !is this && blob.getTeamNum() == this.getTeamNum() && blob.getName() == this.getName() && blob.getPosition().x < thisPos.x && determineZone(this) != determineZone(blob))
		{
			bool added = false;	   		
