...
#include "Heroes_Technology.as";  

const SColor color_hall(255, 211, 249, 193);

const SColor color_tradingpost_base(0xff8888ff);
const SColor color_tradingpost_magic(0xff8787fc);
const SColor color_tradingpost_weapons(0xff8585fa);
const SColor color_tradingpost_armor(0xff8484f7);
...
class WarPNGLoader : PNGLoader
{
	...
	void handlePixel(SColor pixel, int offset)
	{
		...
		// TRADING POST
		else if (pixel == color_tradingpost_base)
		{
			CBlob@ post = spawnBlob( map, "tradingpost", offset, 0 );
			offsets[autotile_offset].push_back( offset );
		}
		else if (pixel == color_tradingpost_magic)
		{
			CBlob@ post = spawnBlob( map, "tradingpost", offset, 0 );
			post.Tag("magic");
			offsets[autotile_offset].push_back( offset );
		}
		else if (pixel == color_tradingpost_weapons)
		{
			CBlob@ post = spawnBlob( map, "tradingpost", offset, 0 );
			post.Tag("weapons");
			offsets[autotile_offset].push_back( offset );
		}
		else if (pixel == color_tradingpost_armor)
		{
			CBlob@ post = spawnBlob( map, "tradingpost", offset, 0 );
			post.Tag("armor");
			offsets[autotile_offset].push_back( offset );
		...
		else if (type >= crap_scroll && type <= super_scroll)
		{
				...
				if (all !is null)
				{
					...
					if (set !is null)
					{
						...
						//Vec2f scrollOffset = Vec2f(map.tilesize * 10, 0);
						//server_MakePredefinedScroll( pos, "carnage" );
						server_MakePredefinedScroll( Vec2f(pos.x, pos.y), "carnage" );
						server_MakePredefinedScroll( Vec2f(pos.x + map.tilesize, pos.y), "telehall" );
